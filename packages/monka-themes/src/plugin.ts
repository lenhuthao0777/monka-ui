...
import { ConfigTheme, MonkaUIPluginConfig } from './types'
...
export const monkaui = (config: MonkaUIPluginConfig = {}) => {
  const {
    themes: themeObject = {},
    defaultTheme = 'light',
  ...
  for (const [themeName, { colors }] of Object.entries(themes)) {
    ...
    for (const [colorName, colorValue] of Object.entries(flatColors)) {
      ...
      try {
        ...
        const monkaColorVariable = `--${prefix}-${colorName}`
        const monkaOpacityVariable = `--${prefix}-${colorName}-opacity`
        // set the css variable in "@layer utilities"
        resolved.utilities[cssSelector]![monkaColorVariable] =
          `${h} ${s}% ${l}%`
        // if an alpha value was provided in the color definition, store it in a css variable
        if (typeof defaultAlphaValue === 'number') {
          resolved.utilities[cssSelector]![monkaOpacityVariable] =
        ...
        resolved.colors[colorName] = ({ opacityVariable, opacityValue }) => {
          ...
          if (!Number.isNaN(+opacityValue)) {
            return `hsl(var(${monkaColorVariable}) / ${opacityValue})`
          }
          ...
          if (opacityVariable) {
            return `hsl(var(${monkaColorVariable}) / var(${monkaOpacityVariable}, var(${opacityVariable})))`
          }
          return `hsl(var(${monkaColorVariable}) / var(${monkaOpacityVariable}, 1))`
        }
      } catch (error: any) {
